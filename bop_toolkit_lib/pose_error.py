...
def vsd(
    ...
    if visib_union_count == 0:
        ...
        for tau in taus:
            ...
            errors.append(e)
    # if 1 < np.sum(np.array(errors)) < 10:
    #     cv2.imshow('Indices', (visib_est.astype(np.uint8) + visib_gt.astype(np.uint8)) * 125)
    #     # cv2.imshow('Indices_gt', visib_gt.astype(np.uint8) * 255)
    #     cv2.waitKey(0)
    return errors
...
def add(R_est, t_est, R_gt, t_gt, pts):
    ...
    views - by Hinterstoisser et al. (ACCV'12). (no symmetry)
...
def adi(R_est, t_est, R_gt, t_gt, pts):
    ...
    - by Hinterstoisser et al. (ACCV'12). (yes symmetry)
